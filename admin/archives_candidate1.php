                                ...
                                $query = $conn->query("SELECT tbl_candidate.candidate_id, tbl_candidate.img, tbl_candidate.position,tbl_partylist.partylist_id, tbl_partylist.party, tbl_candidate.firstname, tbl_candidate.lastname, tbl_candidate.email, tbl_candidate.department, tbl_candidate.year_level, tbl_candidate.gender FROM tbl_candidate INNER JOIN tbl_partylist ON tbl_candidate.partylist_id = tbl_partylist.partylist_id WHERE status = 'deletecandidate' ORDER BY candidate_id");
                                    ...
                                    <tr >
                                       <td width="50"><img src="<?php echo $row['img']; ?>" width="50" height="50" class="img-rounded"></td>
                                            ...
                                            <a id="<?php echo $candidate_id; ?>" href="#delete_archive1<?php echo $candidate_id; ?>"<?php echo $candidate_id?> data-toggle="modal"class="btn btn-danger "><i class="fa fa-trash-o"></i></a> 
                                              <?php require ('delete_archive1_modal.php'); ?> 

                                            <a id="<?php echo $candidate_id; ?>" href="#restore_archive1<?php echo $candidate_id; ?>"<?php echo $candidate_id?> data-toggle="modal"class="btn btn-info "><i class="fa fa-undo "></i></a>
                                           
                                                
                                            </td>
                                              <?php include ('restore_archive1_modal.php')?>
                                    </tr>
