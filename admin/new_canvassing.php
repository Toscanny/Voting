                        ...
                        <div class="panel-body">							                            		
										...
										<table class="table table-striped table-bordered table-hover ">
											...
											while($fetch = $query->fetch_array()){	
													...
													$query2 = $conn->query("SELECT COUNT(tbl_candidate.candidate_id)  as 'percentage1' FROM votes INNER JOIN tbl_candidate
													 ON votes.candidate_id = tbl_candidate.candidate_id WHERE tbl_candidate.position = 'president'" );
													$query3 = $conn->query("SELECT candidate_id, COUNT(*) * 100.0 /  (SELECT COUNT(*) FROM votes) AS Percent FROM votes GROUP BY candidate_id ORDER BY candidate_id;");													
													// $query3 = $conn->query("SELECT candidate_id, total CAST(candidate_id * 100) / total AS 'Percentage2') AS 'Percentage2' FROM votes " );
